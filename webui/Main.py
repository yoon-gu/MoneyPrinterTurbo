...
if "local_video_materials" not in st.session_state:
    # 사용자가 최근 저장한 로컬 소재를 기억해 문안만 바꿔 다시 생성할 때 소재 목록이 사라지지 않게 합니다.
    st.session_state["local_video_materials"] = []

# 언어 파일 로드
locales = utils.load_locales(i18n_dir)

# 제목과 언어 선택을 포함하는 상단 바 생성
...
def open_task_folder(task_id):
    try:
        # task_id는 서버에서 생성한 UUID여야 합니다. 먼저 형식을 검증해 비정상 값이
        # 경로 결합으로 작업 디렉터리 밖에 접근하거나 플랫폼 shell의 특수 문자 해석을
        # 유발하지 않도록 합니다.
        ...
        # UUID 검증이 통과하더라도 최종 경로가 작업 루트 안에 있는지 다시 확인해
        # 향후 호출 측의 task_id 출처가 바뀌더라도 경로 탐색 위험을 막습니다.
        ...
        with middle_config_panel:
            ...
            if llm_provider == "ollama":
                ...
                with llm_helper:
                    ...
                    if config.is_running_in_container():
                        docker_hint = "\n                            > 컨테이너 환경이 감지되었습니다. Base Url을 설정하지 않으면 기본값으로 `http://host.docker.internal:11434/v1`을 사용합니다.\n"
                    tips = f"""
                            ##### Ollama 설정 안내
                            - **API Key**: 아무 값이나 입력해도 됩니다. 예: 123
                            - **Base Url**: 일반적으로 http://localhost:11434/v1 입니다.
                                - `MoneyPrinterTurbo`와 `Ollama`가 **같은 머신에 있지 않다면** `Ollama` 머신의 IP 주소를 입력해야 합니다.
                                - `MoneyPrinterTurbo`를 `Docker`로 배포했다면 `http://host.docker.internal:11434/v1`을 권장합니다.{docker_hint}
                            - **Model Name**: `ollama list`로 확인합니다. 예: `qwen:7b`
                            """
            ...
            if llm_provider == "openai":
                ...
                with llm_helper:
                    tips = """
                            ##### OpenAI 설정 안내
                            > 일부 네트워크 환경에서는 VPN 전체 트래픽 모드가 필요할 수 있습니다.
                            - **API Key**: [공식 사이트에서 발급](https://platform.openai.com/api-keys)
                            - **Base Url**: 공식 OpenAI는 비워 둘 수 있습니다. OpenAI 호환 제공자(예: OpenRouter)를 사용한다면 해당 호환 API 주소를 입력하세요.
                            - **Model Name**: **사용 권한이 있는** 모델을 입력하세요. 호환 제공자를 사용한다면 해당 플랫폼이 지원하는 모델 ID를 입력하세요.
                            """
            ...
            if llm_provider == "moonshot":
                ...
                with llm_helper:
                    tips = """
                            ##### Moonshot 설정 안내
                            - **API Key**: [공식 사이트에서 발급](https://platform.moonshot.cn/console/api-keys)
                            - **Base Url**: https://api.moonshot.cn/v1 로 고정
            ...
            if llm_provider == "qwen":
                ...
                with llm_helper:
                    tips = """
                            ##### Qwen 설정 안내
                            - **API Key**: [공식 사이트에서 발급](https://dashscope.console.aliyun.com/apiKey)
                            - **Base Url**: 비워 둡니다.
            ...
            if llm_provider == "g4f":
                ...
                with llm_helper:
                    tips = """
                            ##### gpt4free 설정 안내
                            > [GitHub 오픈소스 프로젝트](https://github.com/xtekky/gpt4free)로 GPT 모델을 무료로 사용할 수 있지만 **안정성은 낮습니다**.
                            - **API Key**: 아무 값이나 입력해도 됩니다. 예: 123
                            - **Base Url**: 비워 둡니다.
            ...
            if llm_provider == "azure":
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Azure 포털에서 생성](https://portal.azure.com/#view/Microsoft_Azure_ProjectOxford/CognitiveServicesHub/~/OpenAI)
                            - **Base Url**: 비워 둡니다.
            ...
            if llm_provider == "gemini":
                ...
                with llm_helper:
                    tips = """
                            ##### Gemini 설정 안내
                            > 일부 네트워크 환경에서는 VPN 전체 트래픽 모드가 필요할 수 있습니다.
                            - **API Key**: [공식 사이트에서 발급](https://ai.google.dev/)
                            - **Base Url**: 비워 둡니다.
            ...
            if llm_provider == "grok":
                ...
                with llm_helper:
                    tips = """
                            ##### Grok 설정 안내
                            - **API Key**: GrokAPI 키를 입력하세요.
                            - **Base Url**: GrokAPI 기본 URL을 입력하세요.
            ...
            if llm_provider == "deepseek":
                ...
                with llm_helper:
                    tips = """
                            ##### DeepSeek 설정 안내
                            - **API Key**: [공식 사이트에서 발급](https://platform.deepseek.com/api_keys)
                            - **Base Url**: https://api.deepseek.com 로 고정
                            - **Model Name**: deepseek-chat 으로 고정
                            """
            ...
            if llm_provider == "mimo":
                ...
                with llm_helper:
                    tips = """
                            ##### Xiaomi MiMo 설정 안내
                            - **API Key**: [공식 문서에서 발급 안내 확인](https://platform.xiaomimimo.com/docs/zh-CN/quick-start/first-api-call)
                            - **Base Url**: https://api.xiaomimimo.com/v1 로 고정
                            - **Model Name**: 기본값은 mimo-v2.5-pro 이며, 공식 문서에 따라 다른 사용 가능한 모델을 입력할 수 있습니다.
                            """
            ...
            if llm_provider == "modelscope":
                ...
                with llm_helper:
                    tips = """
                            ##### ModelScope 설정 안내
                            - **API Key**: [공식 문서에서 발급 안내 확인](https://modelscope.cn/docs/model-service/API-Inference/intro)
                            - **Base Url**: https://api-inference.modelscope.cn/v1/ 로 고정
            ...
            if llm_provider == "ernie":
                with llm_helper:
                    tips = """
                            ##### Baidu ERNIE 설정 안내
                            - **API Key**: [공식 사이트에서 발급](https://console.bce.baidu.com/qianfan/ais/console/applicationConsole/application)
                            - **Secret Key**: [공식 사이트에서 발급](https://console.bce.baidu.com/qianfan/ais/console/applicationConsole/application)
            ...
            if tips and config.ui["language"] == "ko":
                st.warning(
                    "한국어 UI에서는 OpenAI 호환 제공자, DeepSeek, Moonshot, Ollama 등을 환경에 맞게 선택할 수 있습니다.\n- 국내외 네트워크 접근성과 API 과금 정책을 확인하세요.\n- 로컬 실행을 원한다면 Ollama를 우선 검토하세요."
                )
                st.info(tips)
...
with middle_panel:
    with st.container(border=True):
        ...
        if params.video_source == "local":
            # Streamlit의 파일 유형 검증은 확장자 대소문자에 민감하므로 두 형태를 모두 허용합니다.
    ...
    with st.container(border=True):
        ...
        # 선택한 TTS 서버에 따라 음성 목록 가져오기
        filtered_voices = []

        if selected_tts_server == "siliconflow":
            # SiliconFlow 음성 목록 가져오기
            filtered_voices = voice.get_siliconflow_voices()
        elif selected_tts_server == "gemini-tts":
            # Gemini TTS 음성 목록 가져오기
            filtered_voices = voice.get_gemini_voices()
        elif selected_tts_server == "mimo-tts":
            # Xiaomi MiMo TTS 기본 음색 목록 가져오기
            filtered_voices = voice.get_mimo_voices()
        else:
            # Azure 음성 목록 가져오기
            all_voices = voice.get_all_azure_voices(filter_locals=None)

            # 선택한 TTS 서버에 따라 음성 필터링
            for v in all_voices:
                if selected_tts_server == "azure-tts-v2":
                    # V2 버전 음성 이름에는 "v2"가 포함됩니다.
                    if "V2" in v:
                        filtered_voices.append(v)
                else:
                    # V1 버전 음성 이름에는 "v2"가 포함되지 않습니다.
                    if "V2" not in v:
                        filtered_voices.append(v)
        ...
        # 저장된 음성이 현재 필터링된 음성 목록에 있는지 확인
        if saved_voice_name in friendly_names:
            saved_voice_name_index = list(friendly_names.keys()).index(saved_voice_name)
        else:
            # 없다면 현재 UI 언어를 기준으로 기본 음성을 선택
            for i, v in enumerate(filtered_voices):
                if v.lower().startswith(st.session_state["ui_language"].lower()):
                    saved_voice_name_index = i
                    break

        # 일치하는 음성을 찾지 못하면 첫 번째 음성 사용
        ...
        # V2 버전을 선택했거나 음성이 V2 음성인 경우 서비스 지역과 API key 입력란 표시
        ...
        # SiliconFlow 선택 시 API key 입력란과 설명 표시
        ...
        # Xiaomi MiMo TTS 선택 시 MiMo LLM provider의 API Key를 재사용합니다.
        # 사용자가 MiMo로 문안과 음성을 모두 생성할 때 키 하나만 관리하면 됩니다.
        ...
        if params.bgm_type == "custom":
            ...
            if custom_bgm_file:
                # os.path.exists를 직접 사용하지 않습니다. 사용자가 자주 입력하는 output000.mp3 같은
                # 파일명은 서비스 계층에서 resource/songs 디렉터리로 매핑한 뒤 검증해야 합니다.
                # 서비스 계층은 디렉터리와 파일 유형을 일관되게 제한해 임의 경로 읽기를 막습니다.
                params.bgm_file = custom_bgm_file.strip()
                # st.write(f":red[사용자 지정 배경 음악 선택됨]: **{custom_bgm_file}**")
...
if start_button:
    ...
    if uploaded_audio_file:
        ...
        # 업로드 파일명은 브라우저에서 오므로 디스크 경로에 직접 붙이지 않습니다.
        # 확장자만 보존하고 고정 파일명으로 현재 작업 디렉터리에 저장해 경로 탐색과 특수 문자 문제를 피합니다.
    ...
    if uploaded_files:
        ...
        # 다시 업로드할 때마다 이번에 선택한 소재만 기준으로 삼아 이전 소재가 반복 추가되지 않게 합니다.
        ...
        # 업로드되어 로컬에 저장된 영상 소재를 세션에 기록해 이후 문안만 바꿀 때 재사용합니다.
        st.session_state["local_video_materials"] = persisted_local_materials
    elif params.video_source == "local" and st.session_state["local_video_materials"]:
        # 사용자가 파일을 다시 업로드하지 않은 경우 최근 디스크에 저장된 로컬 소재 목록을 재사용합니다.
        params.video_materials = []
